...
public class BrowserProfile: Profile {
    ...
    class BrowserSyncManager: NSObject, SyncManager {
        ...
        private func statsWrapEngineSync(engine: String, syncThunk: (SyncStatsDelegate) -> SyncResult) -> SyncResult {
            let statsDelegate = SyncEngineStatsObserver()
            statsDelegate.syncEngineWillBeginReport(engine)
            return syncThunk(statsDelegate) >>== effect({ status in
                guard let stats = statsDelegate.syncEngineDidEndReport(engine, status: status) else {
                    return
                }
...
// MARK: Sync Telemetry Reporting
private class SyncStatsReport {
    ...
    init(when: Timestamp, account: FirefoxAccount, didLogin: Bool = false, why: String) {
        self.when = when
    ...
    func addStats(stats: SyncEngineStats, forEngine engine: String) {
...
private class SyncEngineStatsObserver: SyncStatsDelegate {
    ...
    func syncEngine(engine: String, didGenerateUploadStats stats: SyncUploadStats) {
        engineStats?.uploadStats = stats
    }

    func syncEngine(engine: String, didGenerateApplyStats stats: SyncDownloadStats) {
